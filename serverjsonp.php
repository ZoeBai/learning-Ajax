<?php
    ...
    function search(){
        ...
        if(!isset($_GET["number"]) || empty($_GET["number"])){
            echo $jsonp . '({"success":false, "msg":"param error"})';
            return;
        }
        //$staff is a global variable, any function calls it should declare it with "global" keywords first
        global $staff;
        $number = $_GET["number"];
        $result = $jsonp . '({"sucess":false, "msg":"The number is not found"})';
        foreach ($staff as $value) {
            if($value['number'] == $number){
                $result = $jsonp . '({"success":true, "msg":"找到员工: 编号：' . $value["number"] . ', 员工姓名：' . $value["name"].
